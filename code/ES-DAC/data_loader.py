...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        ...
        if self.args['feature_A']:
            ...
            self.args['feature_dims'][1] = self.audio.shape[2]
        if self.args['feature_A_LLD']:
            with open(self.args['feature_A_LLD'], 'rb') as f:
                data_A = pickle.load(f)
            self.audio_LLD = data_A[self.mode]['audio'].astype(np.float32)
            self.args['feature_dims'][3] = self.audio_LLD.shape[2]
    ...
    def __getitem__(self, index):
        sample = {
            ...
            'audio_LLD': torch.Tensor(self.audio_LLD[index]),
            'index': index,
            'id': self.ids[index],
